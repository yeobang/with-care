...
- **획득 단위는 개인이 아니라 단톡방**(§29) — 3~6가구가 모여야 가치가 생기므로 첫 화면의
  단 하나의 행동은 "초대 링크를 단톡방에 붙이기"다. 초대 링크는 7일·최대 5집 다회용이고,
  링크 소지만으로는 멤버가 되지 않는다(부모 멤버 승인 관문). I1·I2·I7의 강제 지점은
  그대로 세션·보드 계층에 남아 있다.

## 알려진 한계 / 하지 않은 것

- **잔여 게이트**: EAS 빌드/스토어 미제출(웹·API는 배포됨 — 아래 배포 절 참고).
  본인인증(PASS)은 스텁 어댑터(사업자 확보 후 교체). 법률 게이트 5건(guardrails §3)은
  변호사 확인 전 정식 출시 불가.
